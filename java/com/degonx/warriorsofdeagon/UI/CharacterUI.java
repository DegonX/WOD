...
import com.degonx.warriorsofdeagon.AttackAndSkills.SkillsActions;
...
public class CharacterUI {
    ...
    private final Button weaponBtn, elementBtn, lordBtn, skillTabBtn, compCmdBtn;
    ...
    private final ProgressBar compHealthBar;
    ...
    public CharacterUI(Character Char, Game game, GameUI gameui, List<Elements> charEle, List<int[]> attacksCooldownList) {
        ...
        gameui.addRemoveFromGameLayout(charIMG, true);
        charIMG.getLayoutParams().height = 315;
        charIMG.getLayoutParams().width = 225;
        ...
        compCmdBtn = game.findViewById(R.id.compCmd);
        compHealthBar = game.findViewById(R.id.compHealthBar);
        ...
        //change companion mode if summoned
        compCmdBtn.setOnClickListener(btn -> {
            Char.compCmd(compMode);
            compMode = !compMode;
        });
    ...
    public void setupEquippedWeaponSelection(Game game, List<Equipments> equipments, int[] equippedWeapons, int bloodSkillLv) {
        int ws = 5;
        if (bloodSkillLv > 0)
            ws++;
        ...
        for (int i = 0; i < ws; i++) {
            ...
            //if weapon equipped
            if (i < 5 && equippedWeapons[i] > -1)
                weaponSelect[i].setBackgroundResource(Weapons.values()[equipments.get(equippedWeapons[i]).equipType].weaponIcon);
    ...
    private void createActivatedSkillIcon(Game game, int ind) {
        ...
        activeSkillsIMG[ind].setImageResource(SkillsData.getActivetedSkillImage(ind + 1));
    ...
    //show hide companions command button and health bar
    public void showHideCompObjects(int health) {
        if (health > 0) {
            compCmdBtn.setVisibility(View.VISIBLE);
            compHealthBar.setVisibility(View.VISIBLE);
        } else {
            compCmdBtn.setVisibility(View.GONE);
            compHealthBar.setVisibility(View.GONE);
        }
    }

    //set companion command button image and send command
    public void setCompCmdBtnIMG(boolean on) {
        if (on)
            compCmdBtn.setBackgroundResource(R.drawable.compattack);
        else
            compCmdBtn.setBackgroundResource(R.drawable.compstay);
    }

    //set companion health bar and set it to max
    public void setCompHealthHBar(int health) {
        compHealthBar.setMax(health);
        compHealthBar.setProgress(health);
    }

    //change companion health bar
    public void updateCompHealthHBar(int health) {
        compHealthBar.setProgress(health);
    }
    ...
    public void updateXPText() {
        charXPTxt.setText(Char.getXPToLevelUp() + "/" + Char.getCharXP());
    }
