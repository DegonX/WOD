...
public class SettingDB extends SQLiteOpenHelper {
    ...
    private static final String KEY_SETTIGNPK = "settingPK";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        if (!tableExists(db)) {
            String CREATE_TABLE_SETTING = "CREATE TABLE " + TABLE_SETTING + "("
                    + KEY_SETTIGNPK + " INTEGER PRIMARY KEY,"
                    + KEY_USERNAME + " STRING,"
    ...
    public void updateSetting(int what, boolean turnB) {
        ...
        if (what == 1)
            db.execSQL("UPDATE " + TABLE_SETTING + " SET " + KEY_MUTEBGM + "='" + turn + "'" + " WHERE " + KEY_SETTIGNPK + "='" + 1 + "'");
        else if (what == 2)
            db.execSQL("UPDATE " + TABLE_SETTING + " SET " + KEY_MUTESOUND + "='" + turn + "'" + " WHERE " + KEY_SETTIGNPK + "='" + 1 + "'");
    ...
    public void updateSavedAccount(String userName, int save) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL("UPDATE " + TABLE_SETTING + " SET " + KEY_USERNAME + "='" + userName + "', " + KEY_SAVEDUSER + "='" + save + "'" + " WHERE " + KEY_SETTIGNPK + "='" + 1 + "'");
        db.close();
    }
    ...
    public boolean checkIfSettingExist() {
        try {
            ...
            Cursor cursor = db.rawQuery("SELECT " + KEY_SETTIGNPK + " FROM " + TABLE_SETTING + " WHERE " + KEY_SETTIGNPK + "='" + 1 + "'", null);
            if (cursor.moveToFirst()) {
                cursor.close();
                db.close();
                return true;
    ...
    public int getBGMState() {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT " + KEY_MUTEBGM + " FROM " + TABLE_SETTING + " WHERE " + KEY_SETTIGNPK + "='" + 1 + "'", null);
        if (cursor.moveToFirst())
    ...
    public int getSoundState() {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT " + KEY_MUTESOUND + " FROM " + TABLE_SETTING + " WHERE " + KEY_SETTIGNPK + "='" + 1 + "'", null);
        if (cursor.moveToFirst())
    ...
    public int getSaveState() {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT " + KEY_SAVEDUSER + " FROM " + TABLE_SETTING + " WHERE " + KEY_SETTIGNPK + "='" + 1 + "'", null);
        if (cursor.moveToFirst())
    ...
    public String getAccount() {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT " + KEY_USERNAME + " FROM " + TABLE_SETTING + " WHERE " + KEY_SETTIGNPK + "='" + 1 + "'", null);
        if (cursor.moveToFirst())
