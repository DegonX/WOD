...
public class AttacksActions {
    ...
    private static boolean checkYRange(Character Char, AttacksType attack, Mobs m) {
        ...
        if (attack.getAttacksRange() != AttacksRanges.MID && attack.getAttackDirection() != AttacksDirection.SURROUND)
            return Char.getCharY() < m.getMobY() + (m.getMobHeight() * 0.6) && Char.getCharY() + (Char.getCharHeight() * 0.1) > m.getMobY();
    ...
    private static boolean checkXRange(Character Char, AttacksType attack, Mobs m) {
        ...
        if (attack.getAttacksRange() == AttacksRanges.SHORT)
            return Char.getCharX() + Char.getCharWidth() + 25 > m.getMobX() && Char.getCharX() - 25 < m.getMobX() + m.getMobWidth();

            //mid range
        else if (attack.getAttacksRange() == AttacksRanges.MID)
            return Char.getCharX() + Char.getCharWidth() + 175 > m.getMobX() && Char.getCharX() - 175 < m.getMobX() + m.getMobWidth();

            //far range
        else
            return Char.getCharX() + Char.getCharWidth() + 700 > m.getMobX() && Char.getCharX() - 700 < m.getMobX() + m.getMobWidth();
    }
    ...
    private static double changeDamageByType(Elements charEle, MobsType mobType) {
        ...
        Log.d(TAG, "Char Element: " + charEle + ", Mob Type: " + mobType);
        ...
        if (charEle == Elements.Fire && mobType == MobsType.Darkness || charEle == Elements.Fire && mobType == MobsType.Metal || charEle == Elements.Ice && mobType == MobsType.Water ||
                ...
                charEle == Elements.Darkness && mobType == MobsType.Light || charEle == Elements.Lightning && mobType == MobsType.Water || charEle == Elements.Lightning && mobType == MobsType.Darkness ||
                charEle == Elements.Earth && mobType == MobsType.Fire || charEle == Elements.Earth && mobType == MobsType.Lightning || charEle == Elements.Wind && mobType == MobsType.Fire ||
                charEle == Elements.Wind && mobType == MobsType.Darkness)
            return 1.25;
                ...
                charEle == Elements.Lightning && mobType == MobsType.Lightning || charEle == Elements.Lightning && mobType == MobsType.Light || charEle == Elements.Earth && mobType == MobsType.Water ||
                charEle == Elements.Earth && mobType == MobsType.Metal || charEle == Elements.Wind && mobType == MobsType.Earth || charEle == Elements.Wind && mobType == MobsType.Metal)
            return 0.75;
        return 1;
    }
    ...
    //rises or  reduce character damage by mob defence
    private static double changeDamageByDefence(double charDamage, int mobDefence) {
        double damage = charDamage / mobDefence;
        if (damage > 1.3)
            damage = 1.3;
        else if (damage < 0.8)
            damage = 0.8;
    ...
    private static void mobTakeDamage(int damage, int atktimes, Attack attack, Character Char, Mobs m) {
        ...
        for (int at = 0; at < atktimes; at++) {
            ...
            //add character damage and increases\decreases character final damage by mob defence
            finalDamage = (damage + Char.statsMixer(1)) * attack.attackNumber() * (Ran.nextDouble() * (1.3 - 0.8) + 0.8) * changeDamageByDefence(damage, m.getMobDefence());
    ...
    public static void executeAttackSkill(int damage, int manaCost, int atktimes, Character Char, List<Mobs> mobs) {
        ...
        for (Mobs m : mobs) {
            ...
            for (int at = 0; at < atktimes; at++) {
                ...
                int finalDamage = (int) (damage * (Ran.nextDouble() * (1.5 - 0.9) + 0.9)) + Char.getCharDamage() * 5;
